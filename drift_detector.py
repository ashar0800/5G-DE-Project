...
def update_baseline_data(new_data_df):
    ...
    try:
        # Save the current baseline as a backup with timestamp
        if os.path.exists(BASELINE_DATA_FILENAME):
            timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
            backup_filename = f"{OUTPUT_DIR}/baseline_backup_{timestamp}.csv"
            os.rename(BASELINE_DATA_FILENAME, backup_filename)
            write_to_log(f"Previous baseline data backed up to {backup_filename}")
